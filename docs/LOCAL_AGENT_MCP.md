...
- `analyze_pdf_comparison_with_ai(run_dir, provider = "", confirm_external_upload = false, excluded_seqs = None, seqs = None, max_sheets = 12, max_zones = 8, model = "", lang = "ru")`
  ...
  - `provider` accepts `gemini`, `deepseek`, or `qwen`; when omitted, `PDFCOMPARE_VISION_PROVIDER` is used, then economical Gemini 3.7 Flash through OpenRouter;
  - reads the engineer's key only from the MCP environment. Gemini uses `OPENROUTER_API_KEY`; DeepSeek uses `DEEPSEEK_API_KEY`; Qwen uses `QWEN_API_KEY`, `QWEN_BASE_URL`, and optionally `QWEN_MODEL`. No API-key tool argument exists;
  ...
  - Gemini returns the actual USD charge reported by OpenRouter. DeepSeek returns its direct/OpenRouter estimates. Qwen returns token usage without a built-in price estimate because Alibaba tariffs depend on region/account. Locally cached sheets count as zero new spend;
  - `seqs` can narrow a confirmed batch. This is the intended way to request maximum-detail Qwen only for selected sheets; omit `seqs` to run Qwen on every eligible sheet. `max_sheets` is 1..500 (default 500, so the ordinary action is «all») and `max_zones` is 1..20.
...
Gemini through OpenRouter — economical default:

```powershell
$env:OPENROUTER_API_KEY = "<your key>"
$env:PDFCOMPARE_VISION_PROVIDER = "gemini"
./scripts/run_mcp.ps1
```
...
To keep a key between restarts, add the same variables to the Windows user environment or to the MCP client's local environment configuration, then fully restart the MCP client. Do not commit the values to this repository. Optional tuning variables include `PDFCOMPARE_GEMINI_VISION_MODEL`, `PDFCOMPARE_GEMINI_TIMEOUT_SEC`, `PDFCOMPARE_GEMINI_MAX_TOKENS`, the corresponding DeepSeek variables, and `PDFCOMPARE_QWEN_TIMEOUT_SEC` / `PDFCOMPARE_QWEN_MAX_TOKENS`. Ordinary PDF comparison remains fully local and does not require any AI key.
