...
- при ручном выборе формата превью переключается на **чистый лист-схему** выбранного формата в реальных пропорциях: рамка чертежа и пунктирный ориентир штампа 185×55 мм в правом нижнем углу — зоны штампов удобно обводить, не глядя на живой чертёж; режим «Авто» возвращает реальную страницу;
- кнопка **«Подложка…»** подставляет любой другой PDF как визуальную основу (например, чистый шаблон листа с рамкой и штампом) — зоны остаются в процентах листа и применяются к сравниваемым документам как обычно; выбранная подложка запоминается между запусками;
