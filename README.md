...
- **Формат** A4–A0 и **ориентация** (вертикальный/горизонтальный): переключение меняет только превью — зоны остаются теми же. Это способ проверить глазами, куда лягут ваши зоны на другом листе;
- в режиме **«Авто»** показывается реальная страница (или подложка); при ручном выборе формата — **чистый лист-схема** в реальных пропорциях: рамка чертежа и пунктирный ориентир штампа 185×55 мм в правом нижнем углу;
- кнопка **«Подложка…»** подставляет любой другой PDF как визуальную основу (например, чистый шаблон листа) — зоны от этого не меняются; выбранная подложка запоминается между запусками;
- **сетка** 5–50 мм, живая подпись размера при рисовании; зоны можно двигать, растягивать за 8 маркеров и удалять (Del);
- **список областей** справа от листа: номер, размер, привязка, удаление;
