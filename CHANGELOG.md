...
- Исправлен откат перегенерации отчётов: исходные файлы и резервные копии сохраняются при ошибках переименования и удаления; повторный откат безопасен.
- Тяжёлые MCP-операции выполняются в отдельном процессе, статус и отмена остаются доступны. Linux-отмена завершает всю проверенную группу процессов; живой worker не объявляется зависшим по одному таймауту ожидания.
- Кэш визуального анализа учитывает содержимое изображений и метаданные листов, исключая устаревшие результаты после перегенерации.
- Векторизация сопоставления листов ускорила синтетический assignment 200×200 примерно в 5 раз (2,37 → 0,45 с).
- Установка и проверки обнаруживают расхождение окружения с hash-lock. Portable ZIP дополнен необходимыми модулями идентификации процессов и проверки окружения.

- **Gemini 3.7 Flash через OpenRouter стал экономичным vision-режимом MCP по умолчанию.** Фактическая стоимость берётся из `usage.cost`; Qwen 3.8 Max остаётся режимом максимальной детализации для всех или выбранных через `seqs` листов. Ключи по-прежнему читаются только из локального окружения, а внешняя передача требует явного подтверждения.

## v0.1.31 - 2026-08-23

### Security
- Автообновление резервирует уникальный временный файл вместо предсказуемого имени и повторно сверяет SHA-256 непосредственно перед запуском установщика. Подменённый, повреждённый или исчезнувший файл не запускается и удаляется.
- CI-аудит зависимостей стал блокирующим: релиз не собирается при известной уязвимости. Сборочные артефакты создаются только для тега релиза или ручного запуска, а параллельные прогоны одного ref отменяют устаревший прогон.

### Fixed
- Нефатальные ошибки GUI, сохранения состояния, drag-and-drop, чтения PDF и live-отчёта больше не теряются бесследно. Оконное приложение ведёт ротируемый диагностический журнал `~/.pdfcompare_local/pdfcompare.log` и корректно освобождает его при закрытии.
- Диапазон OpenCV ограничен текущей основной веткой `>=5.0,<6`, чтобы свободная установка не подтянула будущую несовместимую major-версию. Добавлены стандартные исключения для кэшей Ruff и mypy.

### Verification
- `lint.ps1` — чисто: Ruff и mypy без замечаний.
- Публичный набор — 265 passed, 1 skipped; покрытие core 86,53%.

## v0.1.30 - 2026-08-23

### Security
- Обновлены зафиксированные зависимости после релизного аудита: `cryptography 49.0.0 → 50.0.0` и сборочный `pip 26.1.2 → 26.2.1`. Исправлены `PYSEC-2026-3552` и `PYSEC-2026-3721`; runtime-lock и полный CI-lock синхронизированы.

## v0.1.29 - 2026-08-23

### Added
- **Опциональный визуальный анализ diff через DeepSeek или Qwen в MCP.** `preview_pdf_vision_analysis` без сети показывает точный список допустимых листов и состояние кэша. `analyze_pdf_comparison_with_ai` требует явного `confirm_external_upload=true`, отправляет только JPEG-монтажи сопоставленных пар OLD + NEW с ненулевым diff и создаёт локальные HTML, Markdown, JSON и ZIP. Прежний `analyze_pdf_comparison_with_deepseek` сохранён как совместимый alias.
- Добавленные, удалённые, односторонние, несопоставленные, неизменённые и явно исключённые листы централизованно отбрасываются до построения монтажа и вызова внешнего API. Ответы кэшируются по провайдеру, модели, языку и версии промпта. Личные `DEEPSEEK_API_KEY` или `QWEN_API_KEY` читаются только из окружения MCP и не сохраняются в артефактах; официальный Qwen endpoint проверяется до отправки данных.
- AI-пакет дополнен Markdown-отчётом и интерактивной HTML-матрицей в стиле основного PDFCompare-отчёта. Каждый лист открывается отдельной страницей с полноразмерным PNG-слайдером OLD/NEW, логическими зонами, детальными PNG-кропами, масштабированием, панорамой, нижней навигацией и копированием описания зоны в Markdown. Шумовые зоны показаны при первом открытии и отличаются синим цветом; повторяющий единственный тезис блок «Что изменилось» не создаётся.
- Итог визуального анализа показывает стоимость по фактическим токенам: прямой DeepSeek, тот же inference через OpenRouter, пропорциональную комиссию покупки кредитов и сравнение с пиковым тарифом. Локально закэшированные ответы не считаются новым расходом; cache-hit токены учитываются по отдельной ставке.

### Verification
- Добавлены тесты фильтра допустимых листов, evidence-монтажа, раздельного кэша провайдеров, локального отчёта, двухшагового MCP-подтверждения, проверки Qwen endpoint и отсутствия API-ключа в артефактах.

## v0.1.28 - 2026-08-10

### Added
- **Автоматическое многоступенчатое совмещение смещённых чертежей.** Движок сначала находит крупный перенос на уменьшенном растре, затем уточняет affine-преобразование на полном разрешении. В `summary.json` и HTML сохраняются метод, качество, X/Y-смещение в пикселях и миллиметрах, поворот и масштаб.
- MCP preview явно показывает, что автоматическое совмещение включено; request/status worker сохраняют режим `automatic_multiscale`.

### Changed
- Зоны исключения учитываются при регистрации, чтобы изменения в штампе не искажали совмещение основного чертежа.
- Live-отчёт использует общие стили, переводы и browser runtime готового слайдера, поэтому два режима просмотра больше не расходятся.

### Verification
- Регрессионные тесты проверяют крупный перенос, исключённую фиксированную область, обратную совместимость публичного API и безопасный отказ при несовпадении размеров.

## v0.1.27 - 2026-08-05

### Changed
- **Слайдер отчёта получил компактную рабочую раскладку.** Навигация по листам переехала в нижнюю плавающую листалку; граница OLD/NEW по-прежнему управляется мышью, но нижний ползунок больше не отнимает место у чертежа. В углах полотна постоянно видны метки OLD и NEW, а заметки и подсказка свёрнуты в доступные кластеры.
- Верхняя панель теперь разделяет идентификацию пары листов, масштаб и управление зонами изменений. Убрана неиспользуемая команда `1:1`; bbox включается доступным зелёным switch с сохранением прежнего localStorage-формата.
- Добавлена общая двуязычная панель «Справка» для матрицы, страницы листа и слайдера. Данные справки хранятся единожды в `help-data.js`, поэтому размер отчёта не растёт пропорционально числу листов.

### Verification
- `lint.ps1` — чисто.
- `test.ps1` — 247 passed, 1 skipped; покрытие 88,55%.
- Слайдер проверен в headless Edge на реальном прогоне; inline-JS дополнительно проверен `node --check`.

## v0.1.26 - 2026-08-04

### Added
- **Режим «Игнорировать различия толщины линий».** Для CAD-PDF, повторный экспорт которых меняет вес штрихов без изменения геометрии, добавлен явный переключатель в GUI. Фильтр сопоставляет оси штрихов и подавляет утолщение/утоньшение только на прежнем месте; новые и смещённые линии остаются изменениями. Режим выключен по умолчанию, потому что вес линии может нести смысл.
- Режим доступен в CLI (`--ignore-line-weight`), MCP (`ignore_line_weight`) и при перегенерации отдельных листов. Выбор сохраняется в настройках, истории, `summary.json`, CSV и метаданных HTML-отчёта.

### Проверка
- Проверка на обезличенной регрессионной паре CAD-PDF подтвердила, что ложные рамки от одной только толщины штриха исчезают.
- Отдельные регрессионные тесты подтверждают: утолщение на той же оси подавляется, смещённый и новый штрих сохраняются.
- `lint.ps1` — чисто. `test.ps1` — 247 passed, 1 skipped, покрытие 88,47%.

## v0.1.25 - 2026-08-03

### Fixed
- **Одна ревизионная линия на разных форматах листа больше не распадаётся на добавленный и удалённый лист.** Сопоставление считало размер бумаги жёстким запретом: например, старый чертёж на A1 и его новая публикация на A0 отвергались ещё до проверки почти идентичного текста и изображения. Теперь различие формата допускается только при одинаковых пропорциях листа и очень сильном совпадении содержимого; слабое или неоднозначное совпадение по-прежнему не связывается.
- **Сопоставленные листы разных размеров проходят полноценный diff.** Растры одинаковых пропорций приводятся к меньшему размеру перед ECC-выравниванием, поэтому отчёт показывает реальные изменения, а не `size_mismatch`. Исправление находится в core и одинаково действует в GUI, CLI и MCP.
- **CI снова устанавливает hash-lock с зафиксированной версией pip.** После предварительного обновления pip запуск через `pip.exe` отказывался понижать сам себя до версии из lock-файла. Установка теперь вызывается как `python -m pip`, что является поддерживаемым способом самомодификации pip на Windows.

### Проверка
- Регрессия воспроизведена на реальных одно­листовых PDF: A1 → A0 теперь даёт одну пару `A1 → B1`, статус `matched`, успешное ECC-выравнивание и рассчитанный уровень изменений.
- `lint.ps1` — чисто. `test.ps1` — 243 passed, 1 skipped, покрытие 88,45%.
