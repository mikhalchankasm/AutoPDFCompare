...
class FinalReportTests(unittest.TestCase):
    ...
    def test_slider_report_has_next_prev_and_hover_sheet_picker(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((run_dir / INTERNAL_REPORT_DIR / "report" / "help-data.js").exists())
            self.assertIn('src="../help-data.js"', slider_html)
            self.assertIn('src="../help-data.js"', detail_html)
            ...
            self.assertNotIn('id="oneBtn"', slider_html)
            self.assertIn('role="switch"', slider_html)
            self.assertIn('aria-checked="true"', slider_html)
            self.assertIn('data-color="yellow"', slider_html)
            self.assertIn(".swatch-yellow", slider_html)
